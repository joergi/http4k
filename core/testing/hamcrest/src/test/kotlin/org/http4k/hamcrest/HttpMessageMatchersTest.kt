...
class HttpMessageMatchersTest {
    ...
    @Test
    fun `header - non-nullable string matcher`() = assertMatchAndNonMatch(Request(GET, "/").header("header", "bob").header("header", "bob2"), hasHeader("header", containsString("bob")), hasHeader("header", equalTo("bill")))
    ...
    @Test
    fun `body non-nullable string matcher`() = assertMatchAndNonMatch(Request(GET, "/").body("bob"), hasBody(containsString("bo")), hasBody(containsString("foo")))

    @Test
    fun `body matcher`() = assertMatchAndNonMatch(Request(GET, "/").body("bob"), hasBody("bob"), hasBody("bill"))

    @Test
    fun `json body matcher`() = assertMatchAndNonMatch(Request(GET, "/").body("""{"hello":"world"}"""), hasBody("""{"hello":"world"}"""), hasBody("""{"hello":"w2orld"}"""))
